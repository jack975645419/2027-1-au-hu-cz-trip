...
├── 交通指引.md          ★ 交通指引源文件（落地进城 / 城际火车大巴 / 市内买票），构建时渲染成 guide/
├── guide/index.html    交通指引网页版（GitHub Pages 不渲染 .md，故由 build_site.py 生成）
...
| 脚本 | 作用 |
|---|---|
| `build_site.py` | 渲染整站 → 仓库根目录（含图片压缩）；顺带把根目录 `交通指引.md` 渲染成 `guide/index.html` |
...
- **每日一色**：按城市分组，**同城各天在色相环上等间隔取色**（同城不撞色，不同城市允许重复），箭头、地图小卡片、城内连线共用；底色深浅自动切换文字颜色保证可读。早期用过白→黑灰阶，用户反馈不好看，已废弃。
- **箭头朝向**：旋转角 = 方位角 − 90（漏减会整体歪 90°）；三角与人字两个图形都必须朝 **+x**（SVG 默认方向），否则会有箭头反向。
- **交通点**：机场 / 火车站 / 大巴站常显；相关地铁站缩放到城市级别才显示。
- **图层开关**两项：「交通站点」（机场/火车站/大巴站/地铁站）+「推荐酒店」，**两者默认都关闭**，需手动勾选；景点层、日程箭头常开。
- 连线是**示意直线**，非真实路径规划。
